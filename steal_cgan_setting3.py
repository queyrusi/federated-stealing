...
class StealConditionalGAN(object):
    ...
    def federated_steal(self):
        ...
        round_length = 1  # one update every 10 batch
        i = 0  # Batch iterator
        for epoch in range(self.args.n_epoch):
            for batch in range(self.args.n_batch):
                ...
                i += 1
                if i>=self.args.n_epoch*self.args.n_batch:
                    break
                for model in self.G:
                    model.zero_grad()
            ...
            fixed_p = self.folder + '/federated_model' + '/cGAN_' + str(epoch) + '.png'
            utils.show_cgan_result(federated_G, epoch, self.args, save=True, path=fixed_p)

            print('stealing [%d/%d]' % (epoch, self.args.n_epoch))
            torch.save(federated_G.state_dict(), self.folder + '/federated_model' + ("/G_%d.pth" % (epoch)))
